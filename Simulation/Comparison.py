...
for j in range(n):
    for i in range(0, nsteps - 1):
        if np.all(np.sqrt((position[i][0] - point_x_e[j]) ** 2 + (position[i][1] - point_y_e[j]) ** 2) < radius):
            if A_random[i] > firing_prob_1:
...
for j in range(n):
    for i in range(0, nsteps - 1):
        if np.all(np.sqrt((position[i][0] - point_x_c[j]) ** 2 + (position[i][1] - point_y_c[j]) ** 2) < radius):
            if A_random[i] > firing_prob_1:
...
# std = np.std(density_t)
# for i in range(n):
#     density = sum(density_t[i])
#
# plt.figure(figsize=(6, 6))
# plt.plot(x_t[1], y_t[1], 'x', alpha=0.9)
# plt.plot(f_p_t[:, 0], f_p_t[:, 1], 'x', color='r')
#
